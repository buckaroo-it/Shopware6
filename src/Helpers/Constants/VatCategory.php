...
class VatCategory
{
	...
	{
		if( !is_numeric($percentage) ) return static::NULL_RATE;
		...
		if( $countryIso == 'NL' )
		{
			if( $percentage <= 0 ) return static::ZERO_RATE;
			if( $percentage < 10 ) return static::LOW_RATE;
		...
		if( $countryIso == 'BE' )
		{
			if( $percentage <= 0 ) return static::ZERO_RATE;
			if( $percentage < 10 ) return static::LOW_RATE;
			if( $percentage < 17 ) return static::MIDDLE_RATE;
			return static::HIGH_RATE;
		}
